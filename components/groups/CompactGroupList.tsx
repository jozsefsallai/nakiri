...
const CompactGroupList: React.FC<CompactGroupListProps> = ({ groups, onChange, activeGroup, activeGuild }) => {
  ...
  return (
    <div>
      {guilds && groups.map(group => (
        <div key={group.id}>
          <div
            className={clsx(baseClasses, { 'bg-ayame-secondary-200': activeGroup === group })}
          ...
          <div className="border-l-4 border-gray pl-1 ml-3">
            <GuildList
              guilds={bulkMapGuildMetadata(guilds, group.guilds as IAuthorizedGuild[])}
