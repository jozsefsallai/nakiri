...
const KeywordSearchResultsIndexPage: React.FC<KeywordSearchResultsIndexPageProps> = ({ id }) => {
  ...
  const fetchItems = async ({ page }: IFetcherOptions = {}) => {
    ...
    try {
      const { keywordSearchResults, pagination } = await apiService.keywordSearchResults.getKeywordSearchResults({ id, page });
  ...
  const handleBlacklistVideoActionClick = async (id: string) => {
    ...
    const result = await MySwal.fire({
      ...
      html: (
        <div>
          <p>Where would you like to blacklist the video <strong>{item.title}</strong>?</p>
          <p>
  ...
  const handleBlacklistChannelActionClick = async (id: string) => {
    ...
    const result = await MySwal.fire({
      ...
      html: (
        <div>
          <p>Where would you like to blacklist the channel <strong>{item.uploaderName}</strong>?</p>
          <p>
            <select onChange={(e) => updateTargetGuild(e.target.value)}>
              <option value="">Global</option>
              {guilds?.map(guild => (
    ...
    <DashboardLayout hasContainer title={title}>
      {keyword && (
        <Blacklist
          ...
          actions={guilds === null ? [] : [
            { label: 'Blacklist video', onClick: handleBlacklistVideoActionClick },
            { label: 'Blacklist channel', onClick: handleBlacklistChannelActionClick }
          ]}
        />
      )}

      {keyword === null && !error && <Loading />}
      {keyword === null && error.length > 0 && <MessageBox level={MessageBoxLevel.DANGER} message={error} />}
    </DashboardLayout>
  );
};
