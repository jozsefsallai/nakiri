...
export class MonitoredKeywordsAPIService {
  ...
  public async getMonitoredKeywords(guild: string): Promise<GetMonitoredKeywordsAPIResponse> {
    const url = this.makeGetMonitoredKeywordsUrl(guild);
    return axiosService.get(url).then(res => res.data);
  }

  public async createMonitoredKeyword({ keyword, guildId, webhookUrl }: CreateMonitoredKeywordAPIRequest): Promise<CreateMonitoredKeywordAPIResponse> {
    return axiosService.post(MonitoredKeywordsAPIService.CREATE_MONITORED_KEYWORD_API_URL, {
      keyword,
      guildId,
      webhookUrl
    }).then(res => res.data);
  ...
  public async deleteMonitoredKeyword(id: string): Promise<DeleteMonitoredKeywordAPIResponse> {
  ...
  private makeUpdateMonitoredKeywordUrl(id: string): string {
    return MonitoredKeywordsAPIService.UPDATE_MONITORED_KEYWORD_API_URL.replace(':id', id);
  }

  private makeDeleteMonitoredKeywordUrl(id: string): string {
    return MonitoredKeywordsAPIService.DELETE_MONITORED_KEYWORD_API_URL.replace(':id', id);
  }
}
