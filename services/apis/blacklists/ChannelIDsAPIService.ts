...
export class ChannelIDsAPIService {
  ...
  static GET_CHANNEL_IDS_WITH_GROUP_URL = '/api/lists/youtube/channels?compact=false&group=:group&strict=true&page=:page&limit=:limit';
  static GET_CHANNEL_IDS_WITH_GUILD_URL = '/api/lists/youtube/channels?compact=false&group=:group&guild=:guild&strict=true&page=:page&limit=:limit';
  ...
  public async getChannelIDs({ group, guild, page, limit }: GetChannelIDsAPIRequest): Promise<GetChannelIDsAPIResponse> {
  ...
  private makeGetChannelIDsUrl(group?: string, guild?: string, page?: number, limit?: number): string {
    let url: string;

    if (group) {
      url = guild
        ? ChannelIDsAPIService.GET_CHANNEL_IDS_WITH_GUILD_URL.replace(':group', group).replace(':guild', guild)
        : ChannelIDsAPIService.GET_CHANNEL_IDS_WITH_GROUP_URL.replace(':group', group);
    } else {
