...
export class VideoIDsAPIService {
  ...
  static GET_VIDEO_IDS_WITH_GROUP_URL = '/api/lists/youtube/videos?compact=false&group=:group&strict=true&page=:page&limit=:limit';
  static GET_VIDEO_IDS_WITH_GUILD_URL = '/api/lists/youtube/videos?compact=false&group=:group&guild=:guild&strict=true&page=:page&limit=:limit';
  ...
  public async getVideoIDs({ group, guild, page, limit }: GetVideoIDsAPIRequest): Promise<GetVideoIDsAPIResponse> {
  ...
  private makeGetVideoIDsURL(group?: string, guild?: string, page?: number, limit?: number): string {
    let url: string;

    if (group) {
      url = guild
        ? VideoIDsAPIService.GET_VIDEO_IDS_WITH_GUILD_URL.replace(':group', group).replace(':guild', guild)
        : VideoIDsAPIService.GET_VIDEO_IDS_WITH_GROUP_URL.replace(':group', group);
    } else {
