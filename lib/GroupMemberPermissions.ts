...
export class GroupMemberPermissionsUtil {
  ...
  static canManageGroupEntries(permissions: number): boolean {
    return UserPermissionsUtil.hasPermission(permissions, GroupMemberPermissions.MANAGE_GROUP_ENTRIES);
  }

  static canSeeApiKey(permissions: number): boolean {
    return UserPermissionsUtil.hasPermission(permissions, GroupMemberPermissions.SEE_API_KEY);
  }

  static canManageGroupGuilds(permissions: number): boolean {
    return UserPermissionsUtil.hasPermission(permissions, GroupMemberPermissions.MANAGE_GROUP_GUILDS);
  }

  static canManageGroupMembers(permissions: number): boolean {
    return UserPermissionsUtil.hasPermission(permissions, GroupMemberPermissions.MANAGE_GROUP_MEMBERS);
  }

  static canAccessGuild(groupPermissions: number, guildPermissions): boolean {
    return GroupMemberPermissionsUtil.canManageGroupGuilds(groupPermissions) ||
      UserPermissionsUtil.hasPermission(guildPermissions, GroupMemberGuildPermissions.ACCESS_GUILD);
  }

  static canManageGuildBlacklists(groupPermissions: number, guildPermissions): boolean {
    return GroupMemberPermissionsUtil.canManageGroupGuilds(groupPermissions) ||
      UserPermissionsUtil.hasPermission(guildPermissions, GroupMemberGuildPermissions.MANAGE_GUILD_BLACKLISTS);
  }
}
