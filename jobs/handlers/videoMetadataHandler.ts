...
import {
  YouTubeVideoID,
  IYouTubeVideoID,
} from '@/db/models/blacklists/YouTubeVideoID';
...
const handler: ProcessCallbackFunction<IVideoMetadataRequest> = async (
  job,
  done,
) => {
  ...
  if (process.env.NODE_ENV !== 'production') {
    console.log(
      `[${job.queue.name}] <- received video metadata request for entry`,
  ...
  if (process.env.NODE_ENV !== 'production') {
    console.log(`[${job.queue.name}] -> fetched video metadata for entry`, {
      id: entry.id,
      videoId: entry.videoId,
    });
  }
