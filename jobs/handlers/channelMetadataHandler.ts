...
import {
  YouTubeChannelID,
  IYouTubeChannelID,
} from '@/db/models/blacklists/YouTubeChannelID';
...
const handler: ProcessCallbackFunction<IChannelMetadataRequest> = async (
  job,
  done,
) => {
  ...
  const connection = await db.getTemporaryNamedConnection(
    'channel-metadata-handler',
  );
  ...
  if (process.env.NODE_ENV !== 'production') {
    console.log(
      `[${job.queue.name}] <- received channel metadata request for entry`,
  ...
  if (!metadata) {
    if (process.env.NODE_ENV !== 'production') {
      console.log(`[${job.queue.name}] -> data fetching failed for entry`, {
        id: entry.id,
  ...
  if (process.env.NODE_ENV !== 'production') {
    console.log(`[${job.queue.name}] -> fetched channel metadata for entry`, {
      id: entry.id,
      channelId: entry.channelId,
    });
  }
